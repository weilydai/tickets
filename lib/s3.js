...
exports.serve = function(length, callback) {
    ...
    findLimit(false, function(minNum) {
        for (var i = 0; i < length; i++) {
            ...
            var url = '/' + config.AWS_S3_NAME + '/unsold/' + fileName + '.pdf';
            var signature = sign.generateSignature(url, expire, config.AWS_SECRET_ACCESS_KEY); // get signature
            //Change to prod or test
            var head = 'http://' + config.AWS_S3_NAME + '.s3.amazonaws.com/unsold/' + fileName + '.pdf?AWSAccessKeyId=';
            var path = head + config.AWS_ACCESS_KEY_ID + '&Expires=' + expire + '&Signature=' + signature;
            attachments.push({
...
exports.moveFile = function(files) {
    files.forEach(function(file) {
        var copyParams = {
            Bucket: config.AWS_S3_NAME,
            CopySource: config.AWS_S3_NAME + '/unsold/' + file.filename,
